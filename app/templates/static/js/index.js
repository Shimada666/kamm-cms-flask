...
layui.config({
	...
	var form = layui.form(),
		...
		tab = layui.bodyTab({
			...
			url : "/cms/system/navs" //获取菜单json地址
		});
			...
			success : function(index, layero){
				...
				$(".skins_box .layui-form-radio").on("click",function(){
					...
					if($(this).find("span").text() != "自定义"){
						...
						$("body").removeAttr("class").addClass("main-body "+skinColor+"");
				...
				form.on("submit(changeSkin)",function(data){
					if(data.field.skin != "自定义"){
						...
						$("body").removeAttr("class").addClass("main-body");
					}
					window.sessionStorage.setItem("skinValue",data.field.skin);
					layer.closeAll("page");
				});
				form.on("submit(noChangeSkin)",function(){
					$("body").removeAttr("class").addClass("main-body "+window.sessionStorage.getItem("skin")+"");
					$(".layui-bg-black,.hideMenu,.layui-layout-admin .layui-header").removeAttr("style");
					skins();
					layer.closeAll("page");
				});
...
function donation(){
	layer.tab({
		...
		tab : [{
			title : "微信",
			content : "<div style='padding:30px;overflow:hidden;background:#d2d0d0;'><img style='width: 100%;height: auto' src='/static/images/wechat.png'></div>"
		},{
			title : "支付宝",
			content : "<div style='padding:30px;overflow:hidden;background:#d2d0d0;'><img style='width: 100%;height: auto' src='/static/images/alipay.jpg'></div>"
		}]
	})
}
